...
(() => {
  ...
  const sectionSubtitles = {
    ...
    operations: "ID, статус, сумма и детали операций",
  };
  ...
  const renderOperationsSection = async () => {
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-pagination" id="adminOperationsPagination">
      ...
      <div class="admin-result-card" id="adminOperationsResult"></div>
    `;
    ...
    if (!searchInput || !searchButton || !tableBody || !prevButton || !nextButton || !pageInfo || !resultNode) {
      return;
    }
    ...
    const loadOperationsPage = async (nextPage, nextQuery = query) => {
      ...
      try {
        ...
        query = String(data.query ?? nextQuery ?? "").trim();
