...
(() => {
  ...
  const getTonConnectUi = (() => {
    ...
    return () => {
      ...
      const TonConnectUIClass = window.TON_CONNECT_UI?.TonConnectUI;
  ...
  const bindTonWalletInteractive = async () => {
    ...
    try {
      ...
      if (tonConnectUi) {
        let connectedWallet = tonConnectUi.wallet || null;
        if (!connectedWallet) {
          connectedWallet = await tonConnectUi.connectWallet();
        }
        nextWallet = getWalletFromTonConnectData(connectedWallet);
      }
      ...
      if (!isValidTonWalletAddress(nextWallet)) {
        ...
        nextWallet = normalizeTonWalletAddress(input);
      }
