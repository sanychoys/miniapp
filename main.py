...
async def try_fragment_fulfill_miniapp_history(history_id: int) -> dict:
    ...
    if not purchase_row:
        return {"ok": False, "error": "Purchase history not found"}

    current_status = str(purchase_row.get("status") or "").strip().lower()
    if current_status == "success":
        return {"ok": True, "skipped": True, "error": "already finalized"}
    ...
    if fulfill_result.get("ok"):
        set_miniapp_purchase_history_error(int(purchase_row.get("id") or 0), "", status="pending")
        return fulfill_result

    fail_stage = str(fulfill_result.get("stage") or "unknown")
    fail_status = int(fulfill_result.get("statusCode") or 0)
    fail_error = str(fulfill_result.get("error") or "fragment auto-fulfillment failed").strip()
    fail_debug = f"[stage={fail_stage} status={fail_status}] {fail_error}"
    ...
    if history_id is not None:
        try:
            ...
            if not fulfill_result.get("ok"):
                ...
                logging.warning(
                    "Failed to auto-fulfill miniapp purchase. id=%s error=%s",
                    history_id,
                    error_text,
                )
                try:
                ...
                await safe_callback_answer(
                    callback,
                    f"Автовыдача не выполнена: {error_text}",
                    show_alert=True,
                )
                return
