...
async def handle_webapp_data(message: types.Message):
    ...
    try:
        ...
        await message.answer("Ошибка: некорректные данные из miniapp.")
        return
    ...
    client_price = payload.get("clientPrice") if isinstance(payload.get("clientPrice"), dict) else {}
    client_price_usd = client_price.get("usd")
    client_price_rub = client_price.get("rub")

    message_lines = [
        "🧾 <b>Новая заявка из MINIAPP (sendData, тест без оплаты)</b>",
        f"👤 <b>Покупатель:</b> @{escape(buyer_username)} (ID: <code>{user_id}</code>)",
        f"🎯 <b>Аккаунт:</b> @{escape(target_username)}",
        f"📦 <b>Товар:</b> {escape(amount_label)}",
        f"💳 <b>Сумма (бот):</b> {final_rub:.2f}₽ / {round(final_rub / MINIAPP_USD_RUB_RATE, 2):.2f}$",
    ]

    if isinstance(client_price_usd, (int, float)) and isinstance(client_price_rub, (int, float)):
        message_lines.append(
            f"🖥 <b>Сумма (клиент):</b> {float(client_price_rub):.2f}₽ / {float(client_price_usd):.2f}$"
        )
